...
async function submitBooking(event) {
  ...
  const bookingData = {
    ...
    payment_amount: 0 // Let the backend calculate the amount
  };
  ...
  try {
    ...
    document.getElementById('conf-name').textContent = result.full_name;
    document.getElementById('conf-room').textContent = `${result.room_type} - Room ${result.room_number || 'TBA'}`;
    document.getElementById('conf-checkin').textContent = new Date(result.check_in_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    document.getElementById('conf-checkout').textContent = new Date(result.check_out_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    document.getElementById('conf-amount').textContent = `$${result.payment_amount.toFixed(2)}`;
